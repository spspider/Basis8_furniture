/**
 ******************************************************************************
 * @file         Экспорт проекта ViyarPro.js
 * @author       Kostiuk Dmitry
 * @email        Bazis2Viyar@email.ua
 * @version      5.3
 * @date         2020/01/29
 * @brief        Скрипт для экспорта деталей и фурнитуры из проектов Базис-Мебельщик 8, 9, 10, 11
 ******************************************************************************
 **/
...
function report() {
    ...
    if (DEBUG == false) {
        if(system.fileExists(NOTEPAD_DEFAULT_PATH)) {
			...
			var folderName =exportFileName.substr(0,exportFileName.lastIndexOf('\\'));
			//alert(folderName);
			system.exec("C:/Windows/explorer.exe","/select, \"" + folderName);
        }
        else {
